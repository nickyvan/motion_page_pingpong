...
import { Elastic, Expo, Power1 } from 'gsap/TweenMax';
...
export default class Slideshow {
  ...
  navigate(direction = 'next') {
    ...
    this.tl
      ...
      .staggerTo(
        currentTitleLetters.sort((a, b) => 0.5 - Math.random()),
        0.2,
        {
          ease: Expo.easeOut,
          cycle: {
            x: () =>
              direction === 'next'
                ? getRandomNumber(-800, -400)
                : getRandomNumber(400, 800),
            y: () => getRandomNumber(-40, 40)
          },
          opacity: 0
        },
        0.5 / currentTitleLettersTotal,
        'begin+=0.6'
      )
      .set(
        upcomingImg,
